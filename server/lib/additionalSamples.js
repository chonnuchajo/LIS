...
function validateAdditionalSampleInput(petition, input) {
  ...
  for (const item of input.items) {
    ...
    if (item.weights !== undefined && (!Array.isArray(item.weights) || item.weights.length !== item.quantity || item.weights.some(weight => !Number.isSafeInteger(weight) || weight < 1))) return 'กรุณาระบุน้ำหนักเป็นจำนวนเต็มมากกว่า 0 กรัมให้ครบทุกตัวอย่าง';
