...
- **แหล่งค่า = hybrid** — ดึงค่าสดจากเซนเซอร์ Node-RED (`/temphum`) มา pre-fill แล้วแก้มือได้
- **หน้าโหลดเอกสาร** = scaffold เปล่า (placeholder)

## ของเดิมที่ค้นพบระหว่างออกแบบ (สำคัญ)

มี pipeline เซนเซอร์ฮาร์ดแวร์อยู่แล้ว แต่ **frontend ยังไม่มีอะไรเรียกใช้**:
- `server/routes/temphum.js` — Node-RED `POST /temphum` ยิงค่า `{ board, temp, hum }` เข้ามา
  เรื่อยๆ (เก็บ latest ต่อ board ใน **memory เท่านั้น**), `GET /temphum` คืน array ค่าสด,
  `POST /temphum/save` + `GET /temphum/history` persist ลง `TempHum`
- `server/models/TempHum.js` — `{ board, temp, hum, receivedAt }`
- **ยังไม่มี mapping board→ห้อง, ไม่มีเกณฑ์ pass/fail, ไม่มีผู้บันทึก, ไม่มี seed/persisted board**
  (live endpoint คืน `[]` เมื่อ Node-RED ไม่ได้ยิง)

แนวทาง: หน้าใหม่ **อ่านค่าสดจาก `GET /temphum`** มา pre-fill แต่ **บันทึกลง model ใหม่
`EnvCheck`** (ที่มี room/เกณฑ์/pass-fail/ผู้บันทึก) — แยกจาก `TempHum` ที่เป็น raw sensor log
ไม่แตะ `/temphum` และ `TempHum` เดิม
...
export interface EnvRoom {
  ...
  boardId: string;     // id บอร์ด Node-RED ที่ monitor ห้องนี้ ("" = ยังไม่ map → พิมพ์มือ)
  tempMin: number;     // °C
  tempMax: number;     // °C
...
> `boardId`/`tempMin`/`tempMax`/`humidityMax` เป็นค่า default — ผู้ใช้เติม board id จริง
> และปรับเกณฑ์ได้ในไฟล์เดียว. `boardId: ""` → หน้าจะไม่ pre-fill ห้องนั้น (พิมพ์มือ)
...
- `getLiveTempHum()` → `GET /temphum` คืน `LiveTempHum[]` (`{ board, temp?, hum?, receivedAt }`)
- types `EnvCheckRecord`, `CreateEnvCheckPayload`, `EnvCheckTodaySummary`, `LiveTempHum`

### 4. หน้า `EnvironmentCheckPage.tsx` (ลอกจาก BalanceRoomPage)

- หัวข้อ + วันที่วันนี้ + badge "ตรวจแล้ว x/3" และ "ผ่าน x/3"
- ดึงค่าสด `getLiveTempHum()` ด้วย React Query `refetchInterval` (เช่น 15s) + `refetchOnWindowFocus`
- 2 tab: **บันทึกผล** / **รายการบันทึก**
- tab บันทึกผล: 3 การ์ด (ห้องละใบจาก `ENV_ROOMS`)
  - หา live reading ของห้องด้วย `boardId` (ถ้า `boardId` ว่างหรือไม่เจอ → ไม่มีค่าสด)
  - **pre-fill** ช่องอุณหภูมิ/ความชื้นจากค่าสดครั้งแรกที่ได้ (ยังไม่เคยกรอก/แก้) แล้วแก้มือได้
  - แสดง badge แหล่งค่า: "เซนเซอร์ • อัปเดต HH:MM" หรือ "ไม่มีเซนเซอร์ — กรอกเอง"
    + ปุ่ม "ดึงค่าล่าสุด" (set ค่าช่อง = live ปัจจุบัน)
...
- **บันทึกลง `EnvCheck` เท่านั้น** (ไม่เรียก `/temphum/save` — แยก raw log ออกจาก daily check)
...
- ไม่แก้ `DailyCheck`/`BalanceRoomPage` เดิม และไม่แตะ `/temphum`/`TempHum` (อ่าน GET อย่างเดียว)
- ไม่ทำหน้า config map board→ห้องผ่าน UI — ใส่ `boardId` ใน config โค้ดพอ
