...
export default function ValidationDataGrid({ label, columns, value, onChange, placeholder }: {
  label: string; columns: string[]; value: string; onChange: (value: string) => void; placeholder?: string;
}) {
  const id = useId();
  ...
  return <div className="space-y-3" role="group" aria-labelledby={id}>
    ...
    {raw ? <Textarea aria-label={label} className="min-h-40 font-mono text-sm" value={value} onChange={event => onChange(event.target.value)} placeholder={placeholder} /> : <>
        ...
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y">{rows.map((row, rowIndex) => <tr key={rowIndex} className="hover:bg-accent">
            ...
            {Array.from({ length: width }, (_, column) => {
              ...
              return <td key={column} className="p-2"><Input aria-label={`${label} แถว ${rowIndex + 1} ${columns[column] ?? `ช่องเกิน ${column + 1}`}`} aria-invalid={invalid} title={invalid ? "ต้องเป็นตัวเลขไม่ติดลบและจำนวนช่องตรงกับหัวตาราง" : undefined} inputMode="decimal" className={`min-w-24 tabular-nums ${invalid ? "border-destructive" : ""}`} value={cell} onChange={event => edit(rowIndex, column, event.target.value)} /></td>;
            })}
            <td className="p-2"><Button type="button" size="icon" variant="ghost" aria-label={`ลบ ${label} แถว ${rowIndex + 1}`} onClick={() => writeRows(rows.filter((_, index) => index !== rowIndex))}><Trash2 className="h-4 w-4" /></Button></td>
          </tr>)}</tbody>
        </table>
      ...
      <Button type="button" size="sm" variant="outline" onClick={() => writeRows([...rows, columns.map(() => "")])}><Plus className="mr-2 h-4 w-4" />เพิ่มแถว</Button>
