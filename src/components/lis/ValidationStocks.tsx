...
export default function ValidationStocks({ stocks, levels, onChange }: {
  stocks: ValidationStock[]; levels: PreparationLevel[]; onChange: (stocks: ValidationStock[]) => void;
}) {
  ...
  return <div className="space-y-4">
    ...
    {stocks.map((stock, index) => {
      ...
      const linked = levels.some(level => level.stockId === stock.id);
      return <div key={stock.id} className="space-y-3 rounded-lg border bg-card p-4 shadow-sm">
        ...
        {linked && <p className="text-xs text-muted-foreground">มีระดับเตรียมสารใช้ Stock นี้ เปลี่ยน Stock ของระดับนั้นก่อนลบ</p>}
      </div>;
    })}
  </div>;
}
