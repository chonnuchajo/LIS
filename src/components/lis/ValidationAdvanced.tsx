...
export function PrecisionPanel({ settings, onChange, result }: {
  ...
  return <Card><CardHeader><CardTitle className="text-base">Repeatability และ Intermediate Precision</CardTitle></CardHeader><CardContent className="space-y-4">
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      <label className="space-y-2 text-sm">ฐาน Horwitz: Target level (mg/mL), C (g/g)<Textarea className="min-h-40 font-mono" value={settings.massFractions} onChange={e => update("massFractions", e.target.value)} placeholder={"0.10,0.25\n0.50,0.25\n1.00,0.25"} /><span className="block text-muted-foreground">กรอกหนึ่งแถวต่อระดับ เลือกฐาน C จากวิธีที่อนุมัติ ไม่แปลงจากความเข้มข้นใน vial อัตโนมัติ</span></label>
      <label className="space-y-2 text-sm">ข้อมูลรายวัน: Day, Target level, Actual fortified, Found<Textarea className="min-h-40 font-mono" value={settings.dailyData} onChange={e => update("dailyData", e.target.value)} placeholder={"1,0.50,0.5029,0.5012\n1,0.50,0.5029,0.5031\n2,0.50,0.5029,0.5040"} /><span className="block text-muted-foreground">ความเข้มข้นเป็น mg/mL · หนึ่งแถวต่อตัวอย่างที่เตรียมแยกกัน · จำนวนซ้ำเท่ากันทุกวัน</span></label>
    </div>
...
export function QcPanel({ settings, onChange, result }: {
  ...
  return <Card><CardHeader><CardTitle className="text-base">ผลตัวอย่างและ QC · ส่วนเสริมตามหัวข้อ 11</CardTitle></CardHeader><CardContent className="space-y-4">
    ...
    {settings.enabled && <>
      ...
      <label className="block space-y-2 text-sm">Sample: Weight (mg), Found (mg/mL), Volume (mL), DF, Density (g/mL)<Textarea className="min-h-32 font-mono" value={settings.sampleData} onChange={e => update("sampleData", e.target.value)} placeholder="50.58,0.5034,25,1,0" /><span className="block text-muted-foreground">DF คือการเจือจางเพิ่มเติมเท่านั้น · Density กรอก 0 เมื่อไม่ทราบและรายงานเฉพาะ %w/w; ต้องมีค่าที่วัดได้เพื่อประเมิน %w/v</span></label>
      <div className="grid gap-4 lg:grid-cols-2">
        <label className="space-y-2 text-sm">Standard Check: Found, Actual standard (mg/mL) · 2 ชุด<Textarea className="min-h-32 font-mono" value={settings.standardData} onChange={e => update("standardData", e.target.value)} placeholder={"0.505,0.5028975\n0.503,0.5028975"} /></label>
        <label className="space-y-2 text-sm">Matrix Spike: Found หลังเติม, Unspiked, Added (mg/mL) · 2 ชุด<Textarea className="min-h-32 font-mono" value={settings.spikeData} onChange={e => update("spikeData", e.target.value)} placeholder={"0.515,0.302,0.201159\n0.511,0.302,0.201159"} /><span className="block text-muted-foreground">Unspiked เป็นผลวัดก่อนเติมที่ปรับ dilution ให้เท่ากับสารละลายสุดท้ายแล้ว ไม่ใช้ค่าฉลากแทนโดยอัตโนมัติ</span></label>
      </div>
      <ValidationResults checks={result.checks} />
