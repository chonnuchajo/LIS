...
export default function ValidationPreparation({ stock, stocks, levels, onChange, analyte, method }: {
  ...
  return <div className="space-y-4">
    ...
    <div className="overflow-x-auto rounded-lg border bg-card shadow-sm">
      <table className="w-full text-sm"><thead className="bg-muted text-muted-foreground"><tr>{["Stock ที่ใช้", "Target และหน่วย", "C stock (mg/mL)", "Stock (µL)", "Solvent (µL)", ...(kind === "matrix" ? ["Matrix (µL)"] : []), "Actual (mg/mL)", "Recovery ต่ำ–สูง (%)", ""].map((h, i) => <th key={i} className="p-3 text-left font-medium">{h}</th>)}</tr></thead>
        <tbody className="divide-y">{levels.map((row, i) => ({ row, i })).filter(({ row }) => preparationKind(row) === kind).map(({ row, i }) => {
          ...
          return <tr key={row.id} className="hover:bg-accent">
            ...
            }}>{["mg/mL", "µg/mL", "mg/L"].map(unit => <option key={unit}>{unit}</option>)}</NativeSelect>{row.targetUnit && row.targetUnit !== "mg/mL" && <p className="text-xs text-muted-foreground">= {targetConcentration(row) ?? "—"} mg/mL</p>}<label className="block space-y-1 text-xs text-muted-foreground">Final volume (µL)<Input aria-label={`finalVolume ระดับ ${i + 1}`} type="number" min="0" step="any" value={row.finalVolume} onChange={e => update(row.id, "finalVolume", e.target.value)} /></label></td>
            <td className="min-w-32 p-2 tabular-nums">{plan?.stock.toLocaleString("en-US", { maximumFractionDigits: 6 }) ?? "—"}</td>
            <td className="min-w-28 p-2"><Input aria-label={`aliquot ระดับ ${i + 1}`} type="number" readOnly value={plan?.aliquotUl.toFixed(1) ?? ""} className="font-semibold tabular-nums" /></td>
            <td className="min-w-32 p-2 tabular-nums">{plan ? plan.solventUl.toFixed(1) : <span className="text-destructive">ตรวจ Stock, Target, Final volume และ Matrix</span>}</td>
            ...
            <td className="min-w-40 p-2 tabular-nums"><strong>{result?.actual.toLocaleString("en-US", { maximumFractionDigits: 6 }) ?? "—"}</strong>{stock != null && !result && <p className="text-xs text-destructive">ตรวจค่าและปริมาตรรวม</p>}</td>
