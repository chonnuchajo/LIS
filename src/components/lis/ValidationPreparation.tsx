...
export default function ValidationPreparation({ stock, stocks, levels, onChange, analyte, method, onSave, onSaveTo, matrixEnabled = false }: {
  ...
  useEffect(() => {
    const next = (matrixEnabled ? syncMatrixTargets(levels) : levels).map(row => {
      if (!matrixEnabled && preparationKind(row) === "matrix") return row;
  ...
  }, [levels, stock, stocks, onChange, matrixEnabled]);
  ...
  return <div className="space-y-4">
    ...
    <Button variant="outline" disabled={!levels.length} onClick={() => setWorksheet(preparationWorksheet({ analyte, method }, levels.filter(row => matrixEnabled || preparationKind(row) === "std"), stock, stocks))}>พิมพ์ตารางคำนวณการเตรียมสาร</Button>
    ...
    {(["std", "matrix"] as const).filter(kind => kind === "std" || matrixEnabled).map(kind => <section key={kind} className="space-y-3">
    ...
    {kind === "matrix" && <div className="space-y-3 rounded-lg border bg-card p-4">
      <h4 className="text-sm font-semibold">คำนวณ Matrix จาก %ยา</h4>{autoMatrix && <p className="text-sm text-muted-foreground">{automaticTargets ? `ดึงจาก STD อัตโนมัติ: ต่ำ ${automaticTargets[0]} · กลาง ${automaticTargets[1]} · สูง ${automaticTargets[2]} mg/mL` : "กรอก STD ให้ครบ 5 ระดับที่ไม่ซ้ำกัน เพื่อดึงค่าต่ำ–กลาง–สูงอัตโนมัติ"}</p>}
      ...
      </div><p className="text-sm text-muted-foreground">Matrix (µL) = ระดับอ้างอิง × Final volume (mL) ÷ สัดส่วนยาโดยน้ำหนัก ÷ ความหนาแน่น Matrix · ใช้ระดับสูงจาก STD เป็นระดับอ้างอิงเดียวกันทุกแถว</p><p className="text-sm text-muted-foreground">ตัวอย่าง 25%w/w ระดับอ้างอิง 1 mg/mL, Final volume 1 mL และความหนาแน่น Matrix 1 mg/µL ได้ Matrix 4 µL · ค่า 1 เป็นสมมติฐานในรายงาน เปลี่ยนเป็นค่าที่ทราบจริง หากฉลากเป็น %w/v ต้องกรอกความหนาแน่นตัวอย่างด้วย</p></>}
    </div>}
