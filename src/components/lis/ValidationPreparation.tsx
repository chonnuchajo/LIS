...
export default function ValidationPreparation({ stock, stocks, levels, onChange, analyte, method }: {
  ...
  return <div className="space-y-4">
    ...
    <div className="overflow-x-auto rounded-lg border bg-card shadow-sm">
      <table className="w-full text-sm"><thead className="bg-muted text-muted-foreground"><tr>{["Stock ที่ใช้", "Target และหน่วย", "แผน Stock / Solvent (µL)", "ปิเปตตามแผน (µL)", ...(kind === "matrix" ? ["Matrix (µL)"] : []), "Actual (mg/mL)", "Recovery ต่ำ–สูง (%)", ""].map((h, i) => <th key={i} className="p-3 text-left font-medium">{h}</th>)}</tr></thead>
        <tbody className="divide-y">{levels.map((row, i) => ({ row, i })).filter(({ row }) => preparationKind(row) === kind).map(({ row, i }) => {
          ...
          return <tr key={row.id} className="hover:bg-accent">
            ...
            {(["aliquot", ...(kind === "matrix" ? ["matrix" as const] : [])] as const).map(key => <td key={key} className="min-w-28 p-2"><Input aria-label={`${key} ระดับ ${i + 1}`} type="number" min="0" step="any" readOnly={key === "aliquot"} value={key === "aliquot" ? plan?.aliquotUl.toFixed(1) ?? "" : row[key]} onChange={e => update(row.id, key, e.target.value)} /></td>)}
