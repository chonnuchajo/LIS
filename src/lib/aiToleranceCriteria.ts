export type AiToleranceCriterion = {
  percent: string;
  criteria: string;
  formulation?: string;
};

export const AI_TOLERANCE_CRITERIA: AiToleranceCriterion[] = [
...
function textAfterMatchedPercent(value: string, match: RegExpMatchArray): string {
  return value.slice((match.index ?? 0) + match[0].length).trim().toUpperCase();
}

export function aiPercentFromCommonName(commonName?: string): string | null {
  const match = commonName?.match(/(\d+(?:\.\d+)?)\s*%/);
  return match ? normalizePercent(match[1]) : null;
}

export function aiToleranceCriteriaForCommonName(commonName?: string): string | null {
  const match = commonName?.match(/(\d+(?:\.\d+)?)\s*%/);
  if (!match) return null;
  const percent = normalizePercent(match[1]);
  const afterPercent = textAfterMatchedPercent(commonName ?? "", match);
  ...
  return (specific || AI_TOLERANCE_CRITERIA.find((item) => item.percent === percent && !item.formulation))?.criteria ?? null;
}

export function isAiContentTestItem(testItem?: string): boolean {
  return /(?:%\s*AI|AI\s*content|active\s*ingredient)/i.test(testItem ?? "");
}
