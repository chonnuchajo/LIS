...
export default function PetitionTimelinePage() {
  ...
  const paginationLabel = hasDateFilter ? `ทั้งหมด ${totalCount} รายการก่อนกรองวันที่` : `ทั้งหมด ${totalCount} รายการ`;
  const showAdminPagination = isAdmin && !!data && totalCount > PAGE_SIZE;
  ...
  return <AppLayout title="Timeline คำร้อง"><div className="space-y-4">
    ...
    <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_320px]"><Card className="border-black-50 shadow-none"><CardHeader className="pb-3"><CardTitle className="flex items-center gap-2 text-base"><CalendarClock className="h-4 w-4 text-primary-500" />แผนภาพระยะเวลาคำร้อง</CardTitle></CardHeader><CardContent>
      {showLoading ? <div className="rounded-[10px] border border-dashed border-grey-200 py-12 text-center text-grey-500">กำลังโหลด timeline...</div> : filteredRows.length === 0 ? <div className="rounded-[10px] border border-dashed border-grey-200 py-12 text-center"><p className="text-sm font-medium text-black-500">{hasFilters ? "ไม่พบคำร้องตามตัวกรอง" : "ยังไม่มีคำร้องที่คุณมีสิทธิ์เห็น"}</p><p className="mt-1 text-xs text-grey-500">ลองเปลี่ยนคำค้นหา สถานะ หรือช่วงวันที่</p>{isAdmin && hasDateFilter && <p className="mt-1 text-xs text-grey-500">ช่วงวันที่ถูกกรองจากข้อมูลในหน้าปัจจุบัน ใช้ปุ่มก่อนหน้า/ถัดไปเพื่อไล่ดูหน้าที่เหลือ</p>}{showAdminPagination && <div className="mt-4 flex flex-wrap items-center justify-center gap-3 text-sm"><span className="text-grey-500">หน้า {page} / {totalPages} · {paginationLabel}</span><div className="flex items-center gap-2"><Button variant="primary-outline" size="sm" disabled={page <= 1} onClick={() => setPage((current) => Math.max(1, current - 1))}>ก่อนหน้า</Button><Button variant="primary-outline" size="sm" disabled={page >= totalPages} onClick={() => setPage((current) => Math.min(totalPages, current + 1))}>ถัดไป</Button></div></div>}</div> : <div className="space-y-3"><div className="overflow-x-auto"><div className="min-w-[980px]"><div className="grid grid-cols-[300px_minmax(640px,1fr)] border-b border-black-50 pb-2 text-xs text-grey-500"><div>คำร้อง</div><div className="relative h-8">{ticks.map((tick) => { const left = timelinePercent(tick.at, timelineWindow); return left == null ? null : <div key={tick.key} className={cn("absolute top-0 h-full border-l", tick.major ? "border-grey-300" : "border-grey-100")} style={{ left: `${left}%` }}><span className="ml-1 whitespace-nowrap">{tick.label}</span></div>; })}</div></div>
        ...
        {showAdminPagination && <div className="flex flex-wrap items-center justify-between gap-3 text-sm"><span className="text-grey-500">หน้า {page} / {totalPages} · {paginationLabel}</span><div className="flex items-center gap-2"><Button variant="primary-outline" size="sm" disabled={page <= 1} onClick={() => setPage((current) => Math.max(1, current - 1))}>ก่อนหน้า</Button><Button variant="primary-outline" size="sm" disabled={page >= totalPages} onClick={() => setPage((current) => Math.min(totalPages, current + 1))}>ถัดไป</Button></div></div>}
      </div>}
    </CardContent></Card>
