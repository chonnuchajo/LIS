...
export default function ValidationPage() {
  ...
  const linearReady = !!fit && levels.length === 5 && levels.every(x => x > 0 && parsed[1].rows.filter(r => r[0] === x).length >= 3);
  ...
  const linearityChecks: Check[] = [
    { name: "R²", value: fmt(fit?.r2), criteria: "≥ 0.995 · 5 ระดับ ระดับละ ≥ 3 ครั้ง", pass: linearReady ? fit.r2 >= 0.995 : null },
  ...
  const accuracyChecks: Check[] = [0.1, 0.5, 1].map(level => {
    ...
    const low = level === 1 ? 95 : 90, high = level === 1 ? 105 : 107;
  ...
  const errors = parsed.flatMap((p, i) => p.errors.map(e => `${names[i]} · ${e}`));
  if (recoveryPoints.some(p => p.expected <= 0 || ![0.1, 0.5, 1].includes(p.level))) errors.push("Accuracy: Actual ต้องมากกว่า 0 และระดับเป้าหมายต้องเป็น 0.10, 0.50 หรือ 1.00 mg/mL");
  const invalidAccuracy = errors.some(e => e.startsWith("Accuracy"));
  if (invalidAccuracy) accuracyChecks.forEach(c => { c.pass = null; });
  const passed = checks.filter(c => c.pass === true).length;
  const failed = checks.filter(c => c.pass === false).length;
  const download = () => {
    const report = { title, template: "Cypermethrin WI-06-04-03 · เกณฑ์จากรายงานที่ให้มา", generatedAt: new Date().toISOString(), status: failed || errors.length ? "พบข้อผิดพลาด / ต้องทบทวน" : "รอตรวจสอบ", preparation: { weightMg: prep[0], purityPercent: prep[1], stockVolumeMl: prep[2], aliquotUl: prep[3], finalVolumeUl: prep[4], stock, actual }, rawData: texts, blankArea: blank, calculations: checks, errors };
