...
function ValueFieldEditor({
  ...
  const setOptionFilter = (
    ...
    if (!hasAny) {
      ...
      // Preserve existing subCategories (UI doesn't expose them, but DB may have legacy values)
      const existing = current[opt] ?? {};
      nextFilters = { ...current, [opt]: { ...existing, ...next } };
    }
    onChange({
      ...field,
      optionFilters: Object.keys(nextFilters).length > 0 ? nextFilters : undefined,
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-1.5">
                          ...
                          {(() => {
                            const pts = field.optionFilters?.[opt]?.productTypes ?? [];
                            if (pts.length === 0) return null;
                            const label = pts.map((p) => productTypeLabels[p] ?? p).join(', ');
                            return (
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <Popover>
                            <PopoverTrigger asChild>
                              <button
                                ...
                                className={cn(
                                  ...
                                  (field.optionFilters?.[opt]?.productTypes?.length ?? 0) > 0
                                    ? "text-emerald-600"
                                    : "text-grey-400",
                                )}
                            ...
                            <PopoverContent className="w-64 p-3 space-y-3" align="end">
                              <div className="text-xs font-semibold text-grey-700">แสดงเฉพาะประเภทสินค้า</div>
                                    ...
                                    <label key={pt} className="flex items-center gap-1.5 text-sm">
                                      <Checkbox
                                        ...
                                        onCheckedChange={(v) => {
                                          ...
                                          setOptionFilter(opt, { productTypes: nextPT });
                                        }}
                                      />
                              ...
                              <div className="flex justify-end pt-1">
                                <Button
                                  ...
                                  onClick={() => setOptionFilter(opt, { productTypes: [] })}
                                >
                                  เคลียร์
                                </Button>
                              </div>
