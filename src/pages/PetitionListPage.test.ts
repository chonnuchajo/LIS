...
describe('canUserCreatePetition', () => {
  it('shows the create petition action for viewer-only users when the route is accessible', () => {
    expect(canUserCreatePetition({ role: 'viewer' }, true)).toBe(true);
    expect(canUserCreatePetition({ roles: ['viewer'] }, true)).toBe(true);
  });
