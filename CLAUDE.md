...
- **Two webhook topologies** (only one active at a time):
  - **LINE → LIS (direct)**: LINE's webhook points at `POST /LIS/api/line/webhook` (signature-verified). Optional `LINE_FORWARD_WEBHOOK_URL` relays every webhook onward (raw body + original `X-Line-Signature`) to a downstream handler; when set, LIS **defers the `join`/`follow` reply** to that downstream (both replying to the single-use replyToken would race).
  - **LINE → n8n → LIS (ingest)**: n8n is LINE's webhook target and relays the payload to `POST /LIS/api/line/ingest`, authenticated by the static `LINE_INGEST_SECRET` (header `X-LIS-Ingest-Key` or `?key=`) instead of the LINE signature (n8n may re-serialize the body). Accepts both the raw `{events}` and n8n's `{body:{events}}` wrapper. Join replies are always deferred (n8n owns them). **`LINE_FORWARD_WEBHOOK_URL` MUST be empty here** or LIS→n8n→LIS loops infinitely.
